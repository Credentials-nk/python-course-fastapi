...
class PostRepository:
    ...
    async def search(
        ...
        results = select(PostORM).options(selectinload(PostORM.tags), joinedload(PostORM.author))

        if query:
            results = results.where(PostORM.title.ilike(f"%{query}%"))

        total = (
            await self.db.scalar(select(func.count()).select_from(results.subquery()))
            or 0
        )
        ...
        orm_posts = list(
            (await self.db.execute(results.limit(per_page).offset(offset)))
            .scalars()
            .all()
        )
    ...
    async def create_post(
        ...
        try:
            ...
            if author:
                author_obj = await self.ensure_author(
